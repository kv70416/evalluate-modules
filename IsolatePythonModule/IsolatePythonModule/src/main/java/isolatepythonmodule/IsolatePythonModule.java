...
public class IsolatePythonModule implements ICodeCompilationModule {
    ...
    public Node moduleGUI(Stage mainWindow, Runnable mainSceneRefresh) {
        ...
        loader.setLocation(getClass().getResource("/isolatepythongui/SubMenuGUI.fxml"));
        ...
        try {
            ...
            File pf = lookForInPath("python");
            if (pf == null) pf = lookForInPath("python3");
            if (pf == null) pf = lookForInPath("pyhton2");

            if (pf != null) {
                config.setPythonPath(pf.getAbsolutePath());
                ctrl.setPythonField(config.getPythonPath());
            }

            ctrl.setMainFileField(config.getMainFileName());
